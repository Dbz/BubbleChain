(function() {
  ...
  Game.prototype.start = function(level) {
    // Add bubbles
    this.levelNum         = level;
    this.level            = BubbleChain.levels[this.levelNum - 1];
    this.firstClickColor  = ""; // Used in level 2
  ...
  Game.prototype.beginTimer = function() { // Fires when start-game menu is clicked
    if(this.levelNum == 4 || this.levelNum == 5) {
      this.timeLeft = 8;
      this.startTimer();
    }
  }
  ...
  Game.prototype.changeLevelScore = function(bubble, action) {
    if(this.firstClickColor == "" && action == "click")
      this.firstClickColor = bubble.color;
    this.gameView.changeScore(this.level.scoreChange(bubble, action, this.firstClickColor, this.bubbles));
  }
  ...
  Game.prototype.roundOver = function() {
    ...
    //   return this.bubbles.length == 0;
    // else if(this.levelNum == 4 || this.levelNum == 5)
    //   return this.timeLeft == -1;
    return this.level.isOver(this.bubbles, this.timeLeft, this.firstClickColor);
