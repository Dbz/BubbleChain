(function() {
	...
	var Game = window.BubbleChain.Game = function(context, canvas, gameView) {
	...
	Game.prototype.chainReaction = function(event) {
		...
		if(~bubble && !bubble.expanding) {
			...
			this.changeLevelScore(bubble);
		}
	}
	
	Game.prototype.changeLevelScore = function(bubble) {
		if(this.level == 1) {
			this.levelScore -= 100;
		}
	...
	Game.prototype.setLevelScore = function() {
		if(this.level == 1)
			this.levelScore = 1000;
	}
	
	Game.prototype.findBubble = function(pos) {
		// Deturmines if bubble is clicked on
		var clicked = -1;
		this.bubbles.forEach(function(bubble) {
